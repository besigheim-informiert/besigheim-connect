...
function Inhalt() {
  const me = useMe();
  const istVerein = Boolean(me.data && !me.data.istPlattformOrg);
  const verein = useVerein(istVerein);
  const veranstaltungen = useVeranstaltungen(istVerein);
  ...
  if (me.isError) return <p className="text-destructive">{me.error.message}</p>;
  if (!me.data) return <p className="text-muted-foreground">Wird geladen …</p>;

  return (
    <div className="grid gap-6 md:grid-cols-2">
      {me.data.istPlattformOrg && !me.data.istPlattformAdmin && (
        <p className="md:col-span-2 text-muted-foreground">
          Sie sind Mitglied der Plattform-Organisation, aber nicht
          Administrator. Die Freigabe von Einreichungen ist Administratoren
          vorbehalten.
        </p>
      )}
      {istVerein && (
        <>
            ...
            <CardContent className="p-5 space-y-3">
              ...
              <Button asChild variant="outline" size="sm">
                <Link to="/admin/verein">
                  ...
                  {verein.data ? "Bearbeiten" : "Anlegen"}
                </Link>
              </Button>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-5 space-y-3">
              ...
              {veranstaltungen.isLoading && (
                <p className="text-sm text-muted-foreground">Wird geladen …</p>
              )}
              {veranstaltungen.isError && (
                <p className="text-sm text-destructive">
                  {veranstaltungen.error.message}
                </p>
              )}
              {veranstaltungen.data && (
                <p className="text-sm text-muted-foreground">
                  {kommende.length} kommende,{" "}
                  {veranstaltungen.data.length - kommende.length} vergangene.
                </p>
              )}
        ...
        <Card className="md:col-span-2">
          <CardContent className="p-5 space-y-3">
            <h2 className="font-semibold text-foreground">
              Freigabe von E-Mail-Einreichungen
            </h2>
            <p className="text-sm text-muted-foreground">
              {freigabe.data
